...
```{r}
UWOMAN <- read_excel("C:/Data science/Project D.S/assignment/Assignment3data.xlsx", sheet = "data")
View(UWOMAN)
str(UWOMAN)
...
```{r}
upd_uwoman %>% group_by(relig) %>% summarise(n=n()) %>% mutate(perc=n/sum(n),perc=scales::percent(perc)) %>% filter(relig%in%c("Muslim","RC")) %>% select(-n)
```
...
woman %>% count(fertility_status) %>% mutate(perc=n/sum(n),perc=scales::percent(perc)) %>% filter(fertility_status=="fertile")
```
...
#We fail to reject the null hypothesis since pvalue >0.05, there is no association between education level and religion since the p-value is greater than the level of significance hence not statistically significant

library(crosstable)
crosstable(woman,c(educl,region),by=relig) %>% as_flextable()

#crosstable(woman,c(educl,relig),by=educl )
?crosstab
...
```{r}
shapiro.test(woman$agefm)
plot(agefm~assoc,data=woman)
MW<-t.test(agefm~assoc,data=woman)
MW$p.value
0.1955273

#we fail to reject the null hypothesis since the pvalue is greater than the level of significance hence not statistically significant to support that their is  mean difference between the gr
```
